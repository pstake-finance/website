     ...
     {list.map((item,index)=>
       (
         <VerticalTimelineElement
           ...
           contentStyle={{ background: '#252525', color: '#fff',  borderRadius:'12px'}}
             ...
             {item.monthList.map((month) =>
               (
                 <div key={month.month} className={"mb-5"}>
                   <p className={"font-semibold text-[20px] leading-[30px] text-[#FFFFFF] mb-2"}>
                   {month.month}
                  </p>
                  <ul className="list-disc pl-[25px]">
                    {
                      month.content.map((content,cindex) =>(
                        <li className={"text-[#C6C6C6]"} key={cindex}>{content.text}</li>
                      ))
                    }
                  </ul>
                </div>
