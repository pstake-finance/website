...
const ValidatorsList = () => {
  const { bldData } = useApp();
  const [dataList, setDataList] = useState<ValidatorInfo[]>([]);
  const [updatedTime, setUpdatedTime] = useState<string>("");
  const fetchBldValidatorsData = useAppStore(
    (state) => state.fetchBldValidatorsData
  );
  ...
  useEffect(() => {
    if (validatorsInfo.bld.list.length <= 0) {
  ...
  useEffect(() => {
    if (validatorsInfo.bld.list.length > 0) {
      ...
      setUpdatedTime(dd!);
      setDataList(validatorsInfo.bld.list);
    }
  }, [validatorsInfo]);
      ...
      <div className={"sectionContainer pt-[120px] pb-[100px]"}>
        ...
        <div className={"rounded-xl bg-[#1D1D1F] py-5 px-6 mb-8"}>
          <div className={"flex items-center justify-between"}>
            <ValidatorsDropdown name={"stkBLD"} />
            <p
              className={
            ...
            >
              ...
              {formatNumber(Number(bldData.tvl), 3, 2)} BLD
            </p>
          </div>
        </div>
        ...
        <div className="pb-4">
          ...
          {!validatorsInfo.bld.loader && dataList.length > 0 ? (
